...
static func save_json_file(json_path: String, data: Variant) -> void:
	...
	if file:
		# 使用 JSON.stringify 的默认行为，但在 Godot 4.3+ 中 Dictionary 默认是有序的
		# 确保不设置 sort_keys 为 true（JSON.stringify 的第三个参数默认为 true 会导致按键排序）
		# 语法：stringify(data: Variant, indent: String = "", sort_keys: bool = true, full_precision: bool = false)
		# 我们需要将 sort_keys 设为 false
